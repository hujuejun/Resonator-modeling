...
lambda_step = 0.001;   % Wavelength step in nm
lambda = 1550.5:lambda_step:1551.5;  % Wavelength range in nm
...
L_ring = 100*pi;    % Total length of the ring in um
loss_ring = 1;  % Equivalent waveguide loss in the ring in dB/cm
n_eff = 2.35;    % Effective index of waveguide

% Coupler parameters
k1 = 0.2;    % Amplitude coupling coefficient of the ring coupler #1
t1 = sqrt(1-(abs(k1)).^2);  % Note that a real value is taken
k2 = 0.2;    % Amplitude coupling coefficient of the ring coupler #2
t2 = sqrt(1-(abs(k2)).^2);
L1 = 50*pi;    % Length of ring section 1 between the two couplers in um
...
DataOutput = transpose([lambda; abs(s_t).^2; abs(s_d).^2]);
save(['Add-Drop.txt'],'DataOutput','-ascii');